        ...
        <a-layout>
            <a-layout-content style="min-width: 800px">
                <a-spin :spinning="hotelListLoading">
                    <br>
                    <a-select mode="multiple" size="default" placeholder="请选择商圈" @change="onChangeBizRegion" style="width: 200px">
                        <a-select-option v-for="item in ['全部','XiDan']" :key="item">{{item}}</a-select-option>
                    </a-select>
                    <a-select mode="multiple" size="default" placeholder="请选择星级" @change="onChangeStar" style="width: 200px">
                    ...
                    <a-list item-layout="vertical" size="large" :data-source="tmpHotelList">
                        <a-pagination slot="pagination" showQuickJumper :total="tmpHotelList.totalElements" :defaultCurrent="1" @change="pageChange"></a-pagination>
                        <a-list-item slot="renderItem" :key="index" slot-scope="item,index" @click="jumpToDetails(item.id)" style="width: 100%">
                            ...
                            <a-list-item-meta>
                                ...
                                <template slot="description">
                                    ...
                                    <div v-else>未预订过</div>
                                    <div><a-rate style="font-size: 15px" :value="item.rate" disabled allowHalf/> {{item.rate}}分</div><br>
                                    <div>
